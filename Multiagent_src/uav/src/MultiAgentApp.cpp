...
void MultiAgentApp::SendFormationMessage(void){
    message->SendMessage("Homing");
}

void MultiAgentApp::SendHomeMessage(void){
    message->SendMessage("Formation");
}
...
void MultiAgentApp::ComputePositionControllers(flair::core::Vector3Df &Controller){
    ...
    if(behaviourMode==BehaviourMode_t::Formation && Form_index >= 1){
        ...
        std::vector<float> RelDistY{0,0,0};
            ...
            xFormBkstp->SetValues(IdDrone,Fleet_x,RelDistX,uav_vel.x,0.0);
            xFormBkstp->Update(GetTime());
            ux = xFormBkstp->Output();
        

            yFormBkstp->SetValues(IdDrone,Fleet_y,RelDistY,uav_vel.y,0.0);
            yFormBkstp->Update(GetTime());
            uy = yFormBkstp->Output();



        //std::cout << "                                       Formation Control" << std::endl;


    }

    //std::cout << "\t\t\t\t\t\t\t\t\tForm_index: " << Form_index << endl;
    Controller.x = ux;
    Controller.y = uy;
    Controller.z = uz;
}
